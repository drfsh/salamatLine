<template>
    <div v-if="page===0" class="box2" style="padding-bottom: 20px;">
        <div class="title">آدرس شما</div>
        <div class="flex-center justify-content-between mb-3"
             style="padding: 0 30px;display: flex;justify-content: space-between;">
            ...
            <div>
                <button @click="openNew()" style="font-size: 13px;" class="add-shop m-0">افزودن</button>
            </div>
        </div>
        <loading style="height: 400px" v-if="list==null"></loading>
        <address-card v-for="(v,i) in list" :value="v" :key="i+'key'"></address-card>
    </div>
    <address-edit v-else :address="edit"></address-edit>
</template>
