<template>
    <div class="simple-item" :title="data.title" :class="{'out-of-stock':!data.active}">
        <a :href="'/products/'+data.slug">
            <div class="cover">
                <img class="lozad" :src="data.tiny" :alt="data.title">
                <span class="add-shop-">
...
"></path></svg>
        ...
        <div class="body">
            <a :href="'/products/'+data.slug">
                <div class="title sl">{{ data.title }}</div>
                <div class="model sl"><span>مدل : </span>{{data.subtitle}}</div>
                <price-card></price-card>
            </a>
            <item-product-btns :id="data.id" :slug="data.slug"></item-product-btns>
        </div>
    </div>
</template>
...
import ItemProductBtns from "../holder/buttons/item-product-btns";
export default {
    name: "productItem",
    components: {ItemProductBtns, PriceCard, FeatureCard},
    props: ['data']
}
</script>
