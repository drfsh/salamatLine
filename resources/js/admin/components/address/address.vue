<template>
    <div>

        <div class="flex-center justify-content-between mb-3" style="padding: 0 30px;">
            <div class="flex-center flex-start">
                <div class="ml-2">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M11.9989 14.1714C9.86891 14.1714 8.12891 12.4414 8.12891 10.3014C8.12891 8.16141 9.86891 6.44141 11.9989 6.44141C14.1289 6.44141 15.8689 8.17141 15.8689 10.3114C15.8689 12.4514 14.1289 14.1714 11.9989 14.1714ZM11.9989 7.94141C10.6989 7.94141 9.62891 9.00141 9.62891 10.3114C9.62891 11.6214 10.6889 12.6814 11.9989 12.6814C13.3089 12.6814 14.3689 11.6214 14.3689 10.3114C14.3689 9.00141 13.2989 7.94141 11.9989 7.94141Z" fill="currentColor"/>
                        <path d="M12.0016 22.76C10.5216 22.76 9.03164 22.2 7.87164 21.09C4.92164 18.25 1.66164 13.72 2.89164 8.33C4.00164 3.44 8.27164 1.25 12.0016 1.25C12.0016 1.25 12.0016 1.25 12.0116 1.25C15.7416 1.25 20.0116 3.44 21.1216 8.34C22.3416 13.73 19.0816 18.25 16.1316 21.09C14.9716 22.2 13.4816 22.76 12.0016 22.76ZM12.0016 2.75C9.09164 2.75 5.35164 4.3 4.36164 8.66C3.28164 13.37 6.24164 17.43 8.92164 20C10.6516 21.67 13.3616 21.67 15.0916 20C17.7616 17.43 20.7216 13.37 19.6616 8.66C18.6616 4.3 14.9116 2.75 12.0016 2.75Z" fill="currentColor"/>
                    </svg>
                </div>
                <div class="flex-column">
                    <span>آدرس حمل و نقل</span>
                    <span class="text-noting" v-if="list!==null && list.length==0">شما هنوز آدرسی ثبت نکرده اید</span>
                </div>
            </div>
            <div>
                <button style="font-size: 13px;" class="add-shop m-0">افزودن</button>
            </div>
        </div>
        <loading style="height: 400px" v-if="list==null"></loading>
        <address-card v-for="(v,i) in list" :value="v" :key="i+'key'"></address-card>
    </div>
</template>

<script>

import Loading from "../loading/loading";
import AddressCard from "./card/address-card";
export default {
    name: "test",
    components: {AddressCard, Loading},
    data(){
      return{
          list:null,
      }
    },
    methods:{
        async getList() {
            let {data} = await window.axios.get('/cart/address/list')
            this.list = data;
        }
    },
    created() {
        this.getList()
    }
}
</script>

<style scoped>

</style>
