...
class SubUpdate extends FormRequest
{
    ...
    {
        $sub = Subcategory::where('id', '=', $this->sub)->first();
        
        return [
            'title' => ['required', 'min:2', Rule::unique('subcategories')->ignore($sub->id)],
            'slug' => ['required', 'min:2', 'alpha_dash', Rule::unique('subcategories')->ignore($sub->id)]
        ];
